...
def test_check_and_increment_half_steps(
    ...
    for i in range(3):
        ...
        executor_contract.executeCipherBatch(ZERO_HASH32, [], ZERO_HASH32, 0, [])
...
def test_call_target_function(
    ...
    batch = make_batch(3)
    tx = executor_contract.executeCipherBatch(ZERO_HASH32, batch, ZERO_HASH32, 0, [])
...
def test_emit_event(
    ...
    for batch_index in range(3):
        ...
        tx = executor_contract.executeCipherBatch(ZERO_HASH32, cipher_batch, ZERO_HASH32, 0, [])
...
def test_check_cipher_batch_hash(
    ...
    with brownie.reverts():
        executor_contract.executeCipherBatch(ZERO_HASH32, [], ZERO_HASH32, 0, [])
    with brownie.reverts():
        executor_contract.executeCipherBatch(make_bytes(32), [], ZERO_HASH32, 0, [])
    executor_contract.executeCipherBatch(batch_hash, [], ZERO_HASH32, 0, [])
...
def test_executing_cipher_batch_checks_signatures_minimal(
    ...
    executor_contract.executeCipherBatch(
        cipher_batch_hash, decrypted_transactions, decryption_key, 1, signatures
    )

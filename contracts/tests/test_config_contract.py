...
def test_num_configs_returns_number_of_configs(config_contract, owner):
    assert config_contract.numConfigs() == 1
    schedule_config(
        config_contract,
...
def test_scheduling_adds_new_config(config_contract, owner):
...
def test_scheduling_checks_seamlessness_after_active_config(config_contract, owner):
...
def test_scheduling_checks_seamlessness_after_inactive_config(config_contract, owner):
...
def test_scheduling_checks_batch_span(config_contract, owner):
...
def test_scheduling_resets_new_config(config_contract, owner):
...
def test_only_owner_can_set_next_config(config_contract, owner, non_owner):
...
def test_only_owner_can_schedule_config(config_contract, owner, non_owner):
    config = make_batch_config(start_batch_index=0, start_block_number=100)
    set_next_config(config_contract, config, owner=owner)
    with brownie.reverts():
...
def test_scheduling_must_happen_with_heads_up(
    web3, config_contract, config_change_heads_up_blocks, owner
):
    config = make_batch_config(start_batch_index=0)
    set_next_config(config_contract, config, owner=owner)
...
def test_scheduling_emits_event(config_contract, owner):
    config = make_batch_config(start_batch_index=0)
...
def test_remove_some_keypers_from_next_config(config_contract, owner):
...
def test_remove_all_keypers_from_next_config(config_contract, owner):
    config = make_batch_config(keypers=[make_address() for _ in range(4)])
    set_next_config(config_contract, config, owner=owner)
...
def test_unschedule_single_config(config_contract, owner):
    config = make_batch_config(start_batch_index=0, start_block_number=100)
    schedule_config(config_contract, config, owner=owner)
...
def test_unschedule_multiple_configs(config_contract, owner):
...
def test_unscheduling_nothing(config_contract, owner):
    config = make_batch_config(start_batch_index=0, start_block_number=100)
    schedule_config(config_contract, config, owner=owner)
...
def test_only_owner_can_unschedule(config_contract, owner, non_owner):
    config = make_batch_config(start_batch_index=0, start_block_number=100)
    schedule_config(config_contract, config, owner=owner)
...
def test_unscheduling_must_happen_with_heads_up(
    config_contract, owner, web3, config_change_heads_up_blocks
):
    block_number = web3.eth.blockNumber
    with brownie.reverts():
...
def test_unscheduling_emits_event(config_contract, owner):
    config = make_batch_config(start_batch_index=0, start_block_number=100)
    schedule_config(config_contract, config, owner=owner)

...
library Pairing {
     ...
     */
    function negate(G1Point memory _point)
        internal
        pure
     ...
     */
    function pairing(G1Point[] memory _g1points, G2Point[] memory _g2points)
        internal
        returns (bool)
    {
        require(_g1points.length == _g2points.length, "Point count mismatch.");

        uint256 elements = _g1points.length;
     ...
     */
    function curveMul(G1Point memory _point, uint256 _scalar)
        private
        returns (G1Point memory)
    {
